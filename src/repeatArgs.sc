...
def lengthOfStrings(strings: Seq[String]) = strings foreach (s => println(s"$s -> ${s.length}"))
lengthOfStrings(Seq("a", "bb", "ccc"))

/* with repeated parameters */
def lengthOfStringsR(strings: String*) = strings foreach (s => println(s"$s -> ${s.length}"))
